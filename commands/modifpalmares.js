...
function loadFiches() {
  ...
  try {
    ...
    console.error("❌ Erreur lecture fiches.json :", e.message);
    return {};
  }
}
...
module.exports = {
  ...
  async execute(riza, m, args) {
    ...
    // Vérifier si la personne a une fiche
    const fiches = loadFiches();
    if (!fiches[target]) {
      return riza.sendMessage(m.chat, {
        text: `❌ @${target.split("@")[0]} n'a pas de fiche enregistrée.\n\nUtilise d'abord la commande *!enregistrer* pour créer sa fiche avant de modifier son palmarès.`,
        mentions: [target]
      }, { quoted: m });
    }
...
✅ Palmarès mis à jour avec succès !`;
