...
function initSocialFiche(jid, pseudo, faction) {
  ...
  if (!socials[jid]) {
    socials[jid] = {
      ...
      grade: "Aventurier",
      titre_honorifique: "",
      equipe: "", // Renommé de "guilde" à "équipe"
      coequipiers: [], // Renommé de "coequipiers" pour cohérence
      reputation: {
        peuple: 0,
        autorites: 0
      }
...
module.exports = {
  ...
  async execute(riza, m, args) {
    ...
    const questions = [  
      ...
      { key: "esprit", text: "🧠 Et son *esprit* ?" },  
      { key: "pouvoir", text: "🌀 Enfin, quel est son *niveau de pouvoir* ?" }  
    ];  
...
⚔️ Pour le premier enregistrement, le corps imposé est "Ensemble Usé (Défense : 0)" et l'insigne de cuivre.
...
Une fois terminé, l'admin validera ta fiche manuellement ✅.`,
    ...
    const askNext = async () => {  
      ...
      if (current === 3) { // Question sur la force
        ...
        questions[4].text = `🧠 Et son *esprit* ? (reste: ${reste})`;
      } else if (current === 5) { // Question sur le pouvoir
        const force = parseInt(answers.force) || 0;
        const esprit = parseInt(answers.esprit) || 0;
        const reste = 150 - force - esprit;
        questions[5].text = `🌀 Enfin, quel est son *niveau de pouvoir* ? (reste: ${reste})`;
      }
      ...
      if (current >= questions.length) {  
        ...
        // CORRECTION : Insigne de cuivre avec nom simple seulement
        const corps = [
          "Ensemble Usé (Défense : 0)", 
          "Insigne de cuivre", // CORRIGÉ ICI - plus de description longue
          ""
        ];
        const sorts = ["", "", ""];
        const cartes = ["", "", ""];
